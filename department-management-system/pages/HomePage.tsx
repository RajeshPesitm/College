...
const HomePage: React.FC = () => {
  ...
  const mainNavItems = [
    { to: '/batches/create', title: 'Manage Batches', description: 'Organize students and subjects by batch.', icon: <BatchIcon className="w-12 h-12 text-white" />, color: 'from-blue-500 to-blue-600', hover: 'hover:from-blue-600 hover:to-blue-700' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <Card key={index} className="flex items-center space-x-4">
            <div className="p-3 bg-blue-600 text-white dark:bg-yellow-300 dark:text-black rounded-full">
              {stat.icon}
            </div>
            <div>
              <p className="text-gray-100 dark:text-gray-800 text-sm">{stat.title}</p>
              <p className="text-2xl font-bold text-gray-200 dark:text-white">{stat.value}</p>
            </div>
          </Card>
        ))}
      </div>
